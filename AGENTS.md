...
This is a Flutter personal finance app backed by Supabase. Application code lives in `lib/`: `main.dart` initializes services, `app.dart` defines the app shell, `core/` holds shared services and constants, `models/` contains data models, `providers/` contains Riverpod state and CRUD logic, `features/` contains screens and feature-specific services, and `widgets/` contains reusable UI. Tests live in `test/`, currently starting with `test/widget_test.dart`. Static assets are in `assets/`, web files in `web/`, platform runners in `android/` and `windows/`, and database migrations in `supabase/migrations/`. Product and architecture notes are in `docs/`.
...
- `flutter build appbundle`: build an Android release bundle.
...
Do not commit real secrets from `.env`. Use `.env.example` for required variable names such as `SUPABASE_URL`, `SUPABASE_ANON_KEY`, and `CLAUDE_API_KEY`. Preserve the app's finance safety rules: soft-delete records, derive balances through database functions, and never let AI features modify money without explicit user confirmation.
