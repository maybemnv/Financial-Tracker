...
- **Manual Entry** — Add transactions with account selector, type (debit/credit/transfer/investment), category, merchant
...
- **Finance Agent** — Claude-powered Q&A with structured context (per-account balance, net worth, monthly burn, goal progress)
...
├── features/
│   ├── transactions/          # Transaction list (account filter) + add form (account selector)
...
│   ├── agent/                 # Chat UI + Claude tool-use data gathering
