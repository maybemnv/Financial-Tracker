-- ============================================================================
-- PART 1 of 2 — Owner registry.  Paste into the Supabase SQL Editor and Run.
--
-- Prerequisites (docs/RUNBOOK.md §2), do these FIRST:
--   1. Enable Google OAuth and/or email magic link in Supabase Auth.
--   2. Add your callback/redirect URLs (localhost, Vercel preview, prod).
--   3. Sign in once as yourself to create the auth.users row, then copy that
--      user's UUID from Authentication -> Users.
--   4. TAKE A BACKUP (docs/RUNBOOK.md §1). These migrations are forward-only.
--
-- After this script succeeds, run the owner insert shown at the bottom, THEN
-- run 03_verify.sql, THEN part 2. Part 2 refuses to run without an owner row.
-- ============================================================================
...
-- ============================================================================
-- NOW RUN THIS, with your own UUID from step 3 above.
-- It is deliberately NOT part of the script: it is the one value only you know.
-- ============================================================================
--
--   INSERT INTO app_owner (user_id, note)
--   VALUES ('00000000-0000-0000-0000-000000000000', 'production owner');
--
-- Then confirm exactly one active owner exists:
--
